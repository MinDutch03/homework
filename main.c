...
int main()
{
    ...
    char foreground_path[] = "./images/foreground.png";
    char weather_path[] = "./images/weather.jpg";

    char save_path[] = "./images/new.png";

    // read foreground
    unsigned char *foreground = stbi_load(foreground_path, &width, &height, &channelf, 0);
    if (foreground == NULL)
    {
    ...
    // read weather
    unsigned char *weather = stbi_load(weather_path, &width, &height, &channelw, 0);
    unsigned char *new = stbi_load(weather_path, &width, &height, &channelw, 0);
    if (weather == NULL)
    {
        printf("\nError in loading the image\n");
        exit(1);
